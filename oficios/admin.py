...
class InstituicaoAdmin(MyModelAdmin):
    list_display = ('nome', 'cidade')
    form = FormInstituicao
    model = Instituicao


class TelefonePessoaAdmin_Inline(AjaxSelectAdminStackedInline):
    list_display = ('telefone','obs')
    form = FormTelefonePessoa
    model = TelefonePessoa


class EmailPessoaAdmin_Inline(AjaxSelectAdminStackedInline):
    list_display = ('email','obs')
    model = EmailPessoa


class EnderecoPessoaAdmin_Inline(AjaxSelectAdminStackedInline):
    list_display = ('cep','logradouro','numero')
    form = FormEnderecoPessoa
    model = EnderecoPessoa


@admin.register(Pessoa)
class PessoaAdmin(MyModelAdmin):
    list_display = ('nome', 'instituicao', 'cargo')
    form = FormPessoa
    inlines = [TelefonePessoaAdmin_Inline, EmailPessoaAdmin_Inline, EnderecoPessoaAdmin_Inline]
    search_fields = ('nome',)
...
class OficioAdmin(MyModelAdmin):
    ...
    #
    #                     for pessoa in pessoas:
    #                         emails_destino = list(set(pessoa.contatoentidade_set.all().values_list('email', flat=True)))
    #
    #                         if emails_destino:
    #                             ctx = {
    #                                 'oficio': oficio,
    ...
    #
    #                         else:
    #                             log.error('Pessoa {} não possui e-mail para contato.', pessoa)
    #
    #                     try:
